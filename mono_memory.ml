...
let main () =
	...
	let first_block = get_genesis in
		...
		let rec create_blockchain l =
			match l with 
			| previous::c -> let content = mk_block_content "isd_groupe_4" [] previous (Random.bits ()) 20 in 
							 ...
							 let block = {block_info = info; block_ctt = content} in 
							  if sufficient_pow 20 info.b_id then 
							 	begin
			...
			| _ -> let content = mk_block_content "isd_groupe_4" [] first_block.g_block (Random.bits ()) 20 in 
